...
Nod* inserareInceput(Santier santier, Nod* lista) {
	Nod* nod = (Nod*)malloc(sizeof(Nod)); //dupa "=", alocam memorie
	nod->santier = santier;
	nod->next = lista;
	return nod;
}
...
void stergereLista(Nod** lista) {//la Nod sunt ** pentru ca il transformam in adresa, pt ca modificam din el
	while ((*lista) != NULL) {
...
int main() {
	...
	Santier s3 = initializareSantier("Santier 3", 15, 550);
	lista = inserareInceput(s1, lista);
	lista = inserareInceput(s2, lista);
	lista = inserareInceput(s3, lista);
	lista = inserareInceput(initializareSantier("Santier 4", 10, 280), lista);
	afiseazaLista(lista);
	int suma = 0;
	suma = sumaMuncitori(lista);
	printf("\nIn total avem %d muncitori", suma);
